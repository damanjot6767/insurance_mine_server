...
exports.createPolicyDataThroughtSheet = (0, async_handler_1.asyncHandler)(async (req, res) => {
    ...
    const worker = new worker_threads_1.Worker(path_1.default.resolve(__dirname, './policy-worker.ts'), {
    ...
    worker.on('message', async (message) => {
        if (!responseSent) {
            ...
            if (message.status === "error") {
                return res.status(500).json(new api_response_1.ApiResponse(500, "error", message?.message || "something went wrong!"));
            }
